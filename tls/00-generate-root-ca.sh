...
PASSWORD="${KAFKA_SSL_TRUSTSTORE_PASSWORD:-nosecret}"

CA_C="${TLS_DN_C:-SE}"
CA_ST="${TLS_DN_ST:-Stockholm}"
CA_L="${TLS_DN_L:-Stockholm}"
CA_O="${TLS_DN_O:-zmstone}"
CA_OU="${TLS_DN_OU:-Kafka}"
CA_CN="KafkaTestRootCA"
...
rm -f truststore.jks
keytool -keystore truststore.jks -alias RootCA -import -file ca.pem -storepass $PASSWORD -noprompt
